...
export default function Airbnb ( {addToCart} ) {
    ...
    // render(
        // const isSuperhost = airbnb.host.isSuperhost ? 'isSuperhost' : 'notSoperhost';
      ...
      <div className="airbnbs-holder d-flex mb-5">
                ...
                <div className="airbnb-wrap-inner mr-2 ml-2 card">
                    <div className="image-holder">
                        <mark className={`position-absolute top-0 start-0 pl-2 pr-2 isNotsuperhost ${airbnb.host.isSuperhost ? 'isSuperhost' : 'notSoperhost'}`}>SUPERHOST</mark> 
                        <span className="position-absolute bottom-50 end-0 badge bg-danger text-white">
                                {airbnb.payment.description}
                            </span>
                        <img src={airbnb.image} alt={airbnb.title} className="card-img-top" />
                    </div>
                    <div className="airbnb-content-holder card-body">
                        <div>
                        
                            <h4 className="h4">{airbnb.title}</h4>
                            
                            <p className="lead">Entire {airbnb.houseType} in {airbnb.location.city}, {airbnb.location.country}
